...
def calculate_normalized_laplacian(adj):
    ...
    # Use reciprocal sqrt instead of pow for better numerical stability
    # Avoid pow(d, -0.5) which has gradient = -0.5 * d^(-1.5) that explodes
    # Use 1/sqrt(d + eps) which has better behaved gradients
    epsilon = 1e-3
    d_inv_sqrt = 1.0 / torch.sqrt(d + epsilon)

    # Clamp to prevent any extreme values
    d_inv_sqrt = torch.clamp(d_inv_sqrt, max=100.0)
...
class GraphS4mer(nn.Module):
    ...
    def forward(
        ...
        # Detach adj_mat - gradients flow through edge_weight, not topology
        adj_mat_for_sparse = adj_mat.detach()
        edge_index, edge_weight = torch_geometric.utils.dense_to_sparse(adj_mat_for_sparse)
        # Reattach gradients to edge_weight for backprop through GNN
        edge_weight = edge_weight.detach().requires_grad_(adj_mat.requires_grad)
